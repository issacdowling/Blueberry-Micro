...
if data_path.exists():
  ...
  if instance_config.get("server_url") == None and download == True:
    logging.critical("Critical error, invalid server_url defined in the instance configuration file. Please correct this.")
    exit(0)
  elif download == True:
    server_config = None
    config_query_uri = f"{instance_config.get('server_url')}/{instance_config.get('uuid')}/config"
    logging.debug(f"Pulling config from: {config_query_uri}")
...
if server_config != None:
    ...
    if(instance_config.get("devices") == None):
        logging.critical("Devices configuration not found. Instance configuration malformed. Exiting.")
        exit(0)
...
class Core:
  def __init__(self, path):
    ...
    if core_json.get("device_handler"):
      logging.info(f"Loaded Device Handler: {self.name}")
    else:
      logging.info(f"Loaded Core: {self.name}")
...
logging.info(f"Found pipewire at index {mic_index}")

# Assign and create Wakeword data directory if necessary
ww_path = data_path.joinpath("ww")
if not ww_path.exists():
  logging.debug("Creating Wakeword Data Directory")
  ww_path.mkdir()
  logging.info("Created Wakeword Data Directory")
...
while True:
  ...
  for model_name in prediction.keys():
    ...
    if confidence >= 0.5:
      ...
      logging.debug("Recording: waiting for 1s of silence")
...
# Word preprocessing ###########################################
      ...
      logging.info(f"Cleaned up words: {spoken_words}")
# Intent Recognition ###########################################
      ...
      #Check if we're setting the state of something
        ...
        if getSpeechMatches(devices, device_check=True) and getSpeechMatches(state_keyphrases):
          ...
          for index, device in enumerate(spoken_devices):
            ...
            ### Set the state ##################
            try:
                logging.info(f"Turning {device.friendly_name} {spoken_state}" )
                #Boolean
                if spoken_state == "on":
                  device.on()
